...
def replace_label(df):
    ...
    df.replace(
        {'prognosis': symtoms_dict(df)}, inplace=True)
    return df
...
class DiseasePrediction:
    ...
    def _load_train_dataset(self):
        ...
        df_train = replace_label(df_train)
    ...
    def select_model(self):
        ...
        if self.model_name == 'decision_tree':
            self.clf = DecisionTreeClassifier(criterion =self.config['model']['decision_tree']['criterion'],
                                              max_depth = self.config['model']['decision_tree']['max_depth'],
                                              max_features = self.config['model']['decision_tree']['max_features'])
        elif self.model_name == 'random_forest':
            self.clf = RandomForestClassifier(n_estimators = self.config['model']['random_forest']['n_estimators'],
                                              bootstrap = self.config['model']['random_forest']['bootstrap'],
            ...
            self.clf = GradientBoostingClassifier(n_estimators = self.config['model']['gradient_boost']['n_estimators'],
                                                  ...
                                                  verbose =self.config['model']['gradient_boost']['verbose'],
                                                  ccp_alpha = self.config['model']['gradient_boost']['ccp_alpha'],
                                                  max_depth = self.config['model']['gradient_boost']['max_depth'],
                                                  learning_rate = self.config['model']['gradient_boost']['learning_rate'],
                                                  n_iter_no_change = self.config['model']['gradient_boost']['n_iter_no_change'],
                                                  tol =self.config['model']['gradient_boost']['tol'])
        return self.clf
