...
from train import symtoms_dict
...
def load_model(model_name):
    return load(str(f"./saved_model/{model_name}.joblib"))


def predict_disease(clf, df_test):
    temp_dict = symtoms_dict(df_test)
    result = clf.predict(df_test)

    # get_key(result, symtoms_dict(df_test))
    return result


def prediction_pipeline(symptom_dict, model_name="gradient_boost"):
    df_test = create_dataframe(symptom_dict)

    # Load pre-trained model
    clf = load_model(str(model_name))

    result = predict_disease(clf, df_test)

    print(f"disease index:{result}")
